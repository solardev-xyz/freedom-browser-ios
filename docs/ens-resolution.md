...
**Time bounds.** Every proven method is bounded like desktop's UR calls with the configured timeout (5 s by default; `ENSResolver.myotisDeadline` can cap Myotis lower, tests do). A ready-but-stalled engine (synced with snap peers that answer "0 headers" while one hangs into the engine's 15 s request timeout) therefore costs a lookup 5 s instead of 15 s, and after such a timeout Myotis is skipped for 15 / 30 / 60 s (escalating). The engine's read keeps running in the background and is observed: a read that succeeds after the lookup moved on sweeps the lower-tier results cached meanwhile and resets the cooldown, so the next lookups go back to Myotis. Measured on the simulator with myotis v0.1.10, a serving engine answers a name in 3.7–19 s (it asks peers one after another; one hung peer costs its 15 s request timeout), which is why the budget is not tighter — upstream issue #465.
