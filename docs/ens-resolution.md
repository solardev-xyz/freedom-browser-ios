...
**Time bounds.** Every proven method is bounded like desktop's UR calls: Colibri gets the configured timeout, and Myotis gets the chain-data router's interactive budget (2 s, `ENSResolver.myotisDeadline`) when a later method is enabled, else the configured timeout. A ready-but-stalled engine (synced with snap peers that answer "0 headers" while one hangs into the engine's 15 s request timeout) therefore costs a lookup 2 s instead of 15 s, and after such a timeout Myotis is skipped for 15 / 30 / 60 s (escalating) until it serves again or its availability flips. The engine's read keeps running in the background; only the lookup moves on.
