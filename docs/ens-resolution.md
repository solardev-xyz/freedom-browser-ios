...
**Time bounds.** Every proven method is bounded like desktop's UR calls with the configured timeout (5 s by default; `ENSResolver.myotisDeadline` can cap Myotis lower, tests do). A ready-but-stalled engine (synced with snap peers that answer "0 headers" while one hangs into the engine's 15 s request timeout) therefore costs a lookup 5 s instead of 15 s, and after such a timeout Myotis is skipped for 15 / 30 / 60 s (escalating). The engine's read keeps running in the background and is observed: a read that succeeds after the lookup moved on sweeps the lower-tier results cached meanwhile and resets the cooldown, so the next lookups go back to Myotis. Measured on the simulator with myotis v0.1.10, a serving engine answers a name in 3.7–19 s (it asks peers one after another; one hung peer costs its 15 s request timeout), which is why the budget is not tighter — upstream issue #465. Myotis v0.1.11/v0.1.12 (engine ABI 32, pinned since 2026-09-24) closed that issue: header and state reads are hedged across peers, the pool admits and ranks peers by their announced head, and readiness gates on the engine's `snapServingPeers` instead of the raw pool size, so the stall and the "ready but every read fails" window should no longer occur; the budget and late adoption stay as the safety net.
