...
│   ├── GasOracle.swift              ✅ eth_gasPrice + base-fee floor (EIP-1559 ⇒ M5.7)
...
### 9.1 How the wallet reads chain state

**Every read goes through `ChainDataRouter`** (`docs/chain-data-router.md`), the port of desktop's chain-data router: the chain's policy order — Myotis, Colibri, RPC quorum, direct on Ethereum and Gnosis; quorum, direct elsewhere — with provenance on every answer. This replaced the single-shot `WalletRPC.fanOut` walk of 2025: wallet reads on chains without a light client are now M-of-K verified when the pool has three endpoints, endpoints disagreeing is a visible state, and page-driven reads (the dapp bridge, onchain apps) get a two-second per-source budget with route cooldowns so a slow prover or engine cannot stall a page. `WalletRPC` remains the typed façade; its error semantics are unchanged (deterministic answers short-circuit, `allProvidersFailed` carries the endpoints' errors, JSON-RPC error envelopes never feed quarantine).

`eth_sendRawTransaction` goes through `router.broadcast`: Myotis over devp2p first where it can, then one direct endpoint at a time; an uncertain P2P outcome is terminal and never re-broadcast (`WalletRPC.Error.broadcastUncertain`). The fee quote takes gas price and the latest header from one source, on direct from one URL, so the base-fee floor is never computed against another endpoint's head.
...
**Current state (as of WP21 ship):** M5.1–M5.6 + WP12–WP21 all shipped on `feature/wagmi`. Wallet supports vault setup, user-initiated send for any held asset (native or ERC-20, with ENS forward + reverse + CCIP-Read on the recipient field), receive (QR + copy), the full EIP-1193 dapp surface — connect, sign (`personal_sign` + `eth_signTypedData_v4`), send (`eth_sendTransaction`), chain switch (`wallet_switchEthereumChain`) — active-tab-scoped connected-site UI with a per-site detail view, per-`(origin, contract, selector, chainID)` auto-approve rules (gated to value=0, non-zero selector), slide-to-approve on `ApproveTxSheet`, and per-chain ERC-20 balance display + chain/asset picker on the send flow. CCIP-Read is on by default. WalletRPC iterates on provider-quirk RPC errors (Cloudflare/Ankr-style), short-circuits only on protocol-deterministic answers (revert, `-32602`, `insufficient funds`), and feeds quarantine only from transport/malformed failures. Settings restructured into a per-section hub (Wallet / ENS / RPC). Remaining M5.7 WP is biometric fallbacks (WP17b, deferred). Test count: 293 passing.
...
- **M5.7 — Polish**. Sequenced WPs, deferred items called out below.
  ...
  - **WP20 — Token registry + per-asset balance display** ✅. New `Wallet/Tokens/` directory: `Token` value-type, `TokenRegistry` (built-in static array of 9 entries — ETH/USDC/USDT/DAI/EURC/BZZ on Mainnet, xDAI/xBZZ/EURe on Gnosis — mirroring desktop's `tokens.json` byte-for-byte), `ERC20Coder` (`balanceOf(address)` encode + uint256 decode via `ABIFunctionEncoder`/`ABIDecoder`), `TokenBalanceFetcher` (`withTaskGroup` parallel fetch over the chain's tokens, native via `eth_getBalance`, ERC-20s via `eth_call(balanceOf)` through `WalletRPC`'s untyped `callJSON`). `BalanceFormatter` extended with `formatAmount(wei:decimals:)` (number-only) + `format(wei:token:)` (number + symbol); decimals-aware so 6-decimal USDC and 16-decimal BZZ render correctly. `WalletHomeView`'s single balance card replaced with an "Assets" list — one `AssetRow` per non-zero asset (logo + bold symbol leading, monospaced amount trailing). Empty state when nothing held: "No assets on {chain}." Decimals are trusted from the registry (built-ins don't change) — saves one `decimals()` RPC per token vs the desktop's fetch-live policy. Token logos bundled from desktop's `assets/tokens/` PNGs (`token-eth`, `token-usdc-eth`, etc.); fallback is a tinted symbol-initial circle for tokens shipped without a logo. Multicall3 batching deferred — at 7 tokens the per-call iteration through WP19's resilient `fanOutBody` is fast enough and avoids a struct-array ABI encoder + dual-path test surface. Future: WP21 sending tokens, WP22 custom tokens UI + dapp `transfer()` decoder, possibly multicall3 if scale grows.
  - **WP21a — Settings restructure + wallet view cleanup** ✅. Settings was a single flat `Form`; now an iOS-Settings-style hub with three NavigationLink rows pushing per-section pages: **Wallet** (security badge, show recovery phrase, wipe wallet — moved off `WalletHomeView`), **ENS** (quorum, safety, off-chain CCIP, docs links — what was previously labeled "Advanced" became "Off-chain resolution"), **RPC** (custom RPC override, public RPC providers list). The split between ENS and RPC is the user's mental model — RPC = which providers we hit; ENS = how resolution behaves on top of them. "Network" naming reserved for future Swarm/p2p node settings. `WalletHomeView` strip: removed the security badge, the manual "Lock wallet" button (auto-lock on background still wired via `ContentView`'s scenePhase observer), the Advanced disclosure (recovery phrase + wipe). `WalletLockedView` got the same treatment — the Advanced disclosure is gone there too, and the now-orphan `WalletAdvancedSection` component was deleted. Wipe stays reachable from a locked wallet by going to the top-level Settings → Wallet → Wipe — `vault.wipe()` doesn't require unlock.
  - **WP21b — Receive view (QR + copy)** ✅. New `ReceiveView` reachable from a "Receive" button placed side-by-side with "Send" on `WalletHomeView`. Renders ENS primary name (when set, async via the existing `reverseResolve`), the address as a selectable `AddressPill`, a QR code generated via Apple's `CIFilter.qrCodeGenerator`, and a "Copy address" button that swaps to a checkmark for ~1.5s on tap. QR payload is the plain hex address (not EIP-681) — addresses are chain-agnostic at the bytes layer (same key derives the same address on every EVM chain), and EIP-681 URIs aren't universally recognized by simpler scanners. QR is generated at 10× scale and rendered with `interpolation(.none)` so the modules stay crisp at retina densities. Footer copy explicitly notes the chain-agnostic property: "Send any EVM asset to this address — same address works on Mainnet, Gnosis, and any other EVM chain." No external QR library — CoreImage is enough.
  - **WP21 — Send any asset (chain + asset picker)** ✅. `SendFlowView` refactored from a native-only flow to a token-aware one: takes `(chain, asset)` parameters with mutable local `@State`, defaulted from the caller. The Send button on `WalletHomeView` enters with `(activeChain, native)`; tapping an asset row enters pre-selected. New `AssetPickerView` is pushed onto the send flow's nav stack from a "From" row — lists every held `(chain, asset)` across chains, grouped by chain, with checkmark on the current selection. Push (not modal sheet) avoids stacking sheets inside `WalletSheet`. ERC-20 sends route through `TransactionService.buildSend(token:recipient:amount:)` which translates a logical "send N of T to R" into the on-chain `(to, value, data)` shape — native pass-through, ERC-20 to-the-contract with `transfer(to, amount)` calldata via `ERC20Coder.encodeTransfer`. Same helper is used by both quote prep (so estimateGas sees the right params) and the broadcast itself. `SendReviewView` is token-aware — amount in token units, fee in native, "Total" row only when both share a unit. Picker writes `(chain, asset)` atomically; the only `onChange` handler is on `asset` (clears amount + quote since decimals may differ between assets). `Max` button fills `balance - feeEstimate` for native, full balance for ERC-20. `TokenLogo` view extracted (used by `AssetRow`, `SendFlowView`'s From row, and the picker rows). Future: WP22 — custom tokens UI + dapp `ApproveTxSheet` decoder for `transfer()` calldata.
  - **Deferred — EIP-1559 fee-tier picker**. The legacy `gasPrice` path exists only because Argent's `EthereumTransaction` is legacy-only; building a fee-tier UX on legacy compounds the debt. Gating task: evaluate Swift libraries with type-2 RLP (`viem-swift` evolution worth re-checking later) or roll our own type-2 encoder with a Hardhat-cross-checked vector test rig. Returns to the lineup once the wire format is in shape — `eth_feeHistory` percentiles + slow/normal/fast picker land at the same time. **Interim hardening (freedom-browser-ios#1)**: after a field incident where one lowballing provider priced a tx below `baseFeePerGas` and nonce-queued five sends for an hour, `GasOracle` now cross-checks `eth_gasPrice` against the latest block's base fee and floors at `baseFee × 1.25 + 1 gwei`; unparseable or effectively-zero quotes throw instead of falling back to 0 wei.
