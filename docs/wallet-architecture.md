...
**Current state (as of WP15 ship):** M5.1–M5.6 + WP12/WP13/WP14/WP15 all shipped on `feature/wagmi`. Wallet supports vault setup, user-initiated send (with ENS forward + reverse for the recipient field), the full EIP-1193 dapp surface — connect, sign (`personal_sign` + `eth_signTypedData_v4`), send (`eth_sendTransaction`), chain switch (`wallet_switchEthereumChain`) — and active-tab-scoped connected-site UI with a per-site detail view for revocation. Remaining M5.7 WPs are auto-approve rules (WP16) and biometric fallbacks + slide-to-approve (WP17). Test count: 247 passing.
...
- **M5.7 — Polish**. Sequenced WPs, deferred items called out below.
  ...
  - **WP16 — Auto-approve rules**. `@Model AutoApproveRule(origin, contract, selector, chainID)`, no expiry (forever until revoked). Toggle on `ApproveTxSheet` shown only for non-zero selectors (matches desktop's `dapp-tx.js:130`). `AutoApproveStore` next to `PermissionStore`. Bridge consults the store before parking the sheet; matched rule → silent broadcast. Sign methods never auto-approve (§7 invariant). Rules surface in `ConnectedSiteDetailView` (the WP15 home for revocation) — light up the "auto" chip on the home card and add a rules subsection to the detail view. Optional bundle: tiny ERC-20 selector→label map ("Token transfer", "Token approval") for legibility on common selectors.
  - **WP17 — Biometric fallbacks + slide-to-approve**. LAContext availability check, clearer copy when Face ID is disabled, passcode-only fallback path after 3 biometric failures. `SlideToApprove` SwiftUI gesture component for `ApproveTxSheet` only (connect / sign stay tap-to-approve per §8).
