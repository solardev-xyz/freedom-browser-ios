...
**Current state (as of WP17a ship):** M5.1–M5.6 + WP12/WP13/WP14/WP15/WP16/WP17a all shipped on `feature/wagmi`. Wallet supports vault setup, user-initiated send (with ENS forward + reverse for the recipient field), the full EIP-1193 dapp surface — connect, sign (`personal_sign` + `eth_signTypedData_v4`), send (`eth_sendTransaction`), chain switch (`wallet_switchEthereumChain`) — active-tab-scoped connected-site UI with a per-site detail view, per-`(origin, contract, selector, chainID)` auto-approve rules (gated to value=0, non-zero selector), and slide-to-approve on `ApproveTxSheet`. Remaining M5.7 WP is biometric fallbacks (WP17b — split out from the original WP17, deferred). Test count: 261 passing.
