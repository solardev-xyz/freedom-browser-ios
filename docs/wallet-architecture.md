...
- **`ENSResolver`** ✅ (WP14): forward and reverse lookup wired through. `resolveAddress(name:)` calls `addr(bytes32)` through the same `QuorumWave` consensus pipeline as the browser's contenthash path — same threat model (ENS-forge), same M-of-K verification. `reverseResolve(address:)` is single-shot via the Universal Resolver's `reverse(bytes,uint256)`, iterating the pool's providers on transport / RPC-error failure (Cloudflare's `-32603`, Ankr's missing-API-key `-32000`, etc.) so a single flaky RPC doesn't poison the cache; only an empty `primary` after a successful response is a definitive nil. UR self-verifies forward(name) == address per ENSIP-1, so reverse doesn't need its own consensus wave — it's decorative on the UI side anyway. **Known gap: off-chain (CCIP-Read) primary names.** Addresses whose primary name resolves via EIP-3668 (`OffchainLookup` revert pointing at HTTP gateways — Namestone, ENS V3 CCIP, etc.) silently show no subtitle today. Wiring CCIP into the reverse path would mirror what `CCIPResolver.swift` does for forward content (off-chain GET, ABI-decode payload, callback-via-eth_call), gated behind the same `settings.enableCcipRead` toggle. Tracked as a follow-up — reverse is decorative, so the failure mode is a missing subtitle rather than a missing recipient.
...
**Current state (as of WP14 ship):** M5.1–M5.6 + WP12/WP13/WP14 all shipped on `feature/wagmi`. Wallet supports vault setup, user-initiated send (with ENS forward + reverse for the recipient field), and the full EIP-1193 dapp surface — connect, sign (`personal_sign` + `eth_signTypedData_v4`), send (`eth_sendTransaction`), and chain switch (`wallet_switchEthereumChain`). Remaining M5.7 WPs are auto-approve rules (WP15), biometric fallbacks + slide-to-approve (WP16), and connected-sites settings polish (WP17). Test count: 247 passing.
...
- **M5.7 — Polish**. Sequenced WPs, deferred items called out below.
  ...
  - **WP15 — Auto-approve rules**. `@Model AutoApproveRule(origin, contract, selector, chainID)`, no expiry (forever until revoked). Toggle on `ApproveTxSheet` shown only for non-zero selectors (matches desktop's `dapp-tx.js:130`). `AutoApproveStore` next to `PermissionStore`. Bridge consults the store before parking the sheet; matched rule → silent broadcast. Sign methods never auto-approve (§7 invariant). Optional bundle: tiny ERC-20 selector→label map ("Token transfer", "Token approval") for legibility on common selectors.
  - **WP16 — Biometric fallbacks + slide-to-approve**. LAContext availability check, clearer copy when Face ID is disabled, passcode-only fallback path after 3 biometric failures. `SlideToApprove` SwiftUI gesture component for `ApproveTxSheet` only (connect / sign stay tap-to-approve per §8).
  - **WP17 — Connected-sites settings polish**. Dedicated `ConnectedSitesView` from a NavigationLink off `WalletHomeView` — favicon, `OriginIdentity.displayString`, granted-on date, last-used relative time, account address, auto-approve rules per origin, bulk "Disconnect all". Replaces the minimal list shipped in WP9.
