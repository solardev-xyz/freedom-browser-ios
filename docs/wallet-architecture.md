...
- **`ENSResolver`** (pending, M5.7 WP14): the wallet's Send-to-ENS-name flow reuses the consensus pipeline. `ENSResolver.resolveContent` today does the `contenthash(bytes32)` call for the browser; M5.7 adds (a) `addr(bytes32)` for forward recipient resolution (`vitalik.eth → 0x…`) through the same `QuorumWave`, and (b) Universal Resolver's single-call `reverse(bytes, uint256)` for reverse lookup (`0x… → vitalik.eth`) shown as a display-only subtitle on the review/approval sheets. Forward resolution is the one place the wallet uses the full consensus wave, because the threat model there *is* ENS-forge; reverse lookup is decorative + has its forward verification baked into UR's contract, so the same path is fine.
...
- **M5.7 — Polish**. Sequenced WPs, deferred items called out below.
  ...
  - **WP14 — ENS recipient resolution + reverse lookup**. Forward (`vitalik.eth → 0x…`) via `addr(bytes32)` through the same `QuorumWave` consensus pipeline as content resolution, in both `SendFlowView` (debounced, "via foo.eth" subtitle). Reverse (`0x… → vitalik.eth`) via the ENS Universal Resolver's single-call `reverse(bytes, uint256)` (matches desktop `ens-resolver.js`), shown as recipient subtitle on `SendReviewView` and `ApproveTxSheet`. Includes `.box` per origin-identity parity. Display-only; canonical recipient stays the address.
