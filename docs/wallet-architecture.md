...
**Current state (as of WP16 ship):** M5.1–M5.6 + WP12/WP13/WP14/WP15/WP16 all shipped on `feature/wagmi`. Wallet supports vault setup, user-initiated send (with ENS forward + reverse for the recipient field), the full EIP-1193 dapp surface — connect, sign (`personal_sign` + `eth_signTypedData_v4`), send (`eth_sendTransaction`), chain switch (`wallet_switchEthereumChain`) — active-tab-scoped connected-site UI with a per-site detail view, and per-`(origin, contract, selector, chainID)` auto-approve rules (gated to value=0, non-zero selector). Remaining M5.7 WP is biometric fallbacks + slide-to-approve (WP17). Test count: 261 passing.
