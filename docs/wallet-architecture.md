...
**Current state (as of WP21b ship):** M5.1–M5.6 + WP12–WP20 + WP21a + WP21b all shipped on `feature/wagmi`. Wallet supports vault setup, user-initiated send (with ENS forward + reverse + CCIP-Read on both for the recipient field), receive (QR + copy), the full EIP-1193 dapp surface — connect, sign (`personal_sign` + `eth_signTypedData_v4`), send (`eth_sendTransaction`), chain switch (`wallet_switchEthereumChain`) — active-tab-scoped connected-site UI with a per-site detail view, per-`(origin, contract, selector, chainID)` auto-approve rules (gated to value=0, non-zero selector), slide-to-approve on `ApproveTxSheet`, and per-chain ERC-20 balance display (read path only — sending tokens is WP21). CCIP-Read is on by default. WalletRPC iterates on provider-quirk RPC errors (Cloudflare/Ankr-style), short-circuits only on protocol-deterministic answers (revert, `-32602`, `insufficient funds`), and feeds quarantine only from transport/malformed failures. Settings restructured into a per-section hub (Wallet / ENS / RPC). Remaining M5.7 WP is biometric fallbacks (WP17b, deferred). Test count: 290 passing.
